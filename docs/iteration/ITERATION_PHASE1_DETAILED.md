...
适用范围：本计划面向 [ITERATION_PLAN.md](ITERATION_PLAN.md#L34-L56) 的 Phase 1。目标是在 Phase 0 的“工具可用闭环”基础上，把 **契约（schema/错误码/截断）** 与 **FilesystemMiddleware（state + reducer）** 固化为 Rust 侧可扩展（Trait-first）、可测试、可持续演进的公共能力层，为 Phase 1.5+ 的 runtime/provider/memory 等打基础。

## 0. 当前系统基线（Phase 1 启动时的真实状态）

以当前代码为 ground truth，Phase 1 的关键交付已落地（本节用于校准“文档/计划”与实现一致）：

- FilesystemState + delta + reducer：见 [state.rs](../../crates/deepagents/src/state.rs)
- FilesystemMiddleware：工具后回填 `AgentState.filesystem` 并产出 delta：见 [filesystem.rs](../../crates/deepagents/src/middleware/filesystem.rs)
- 工具输入 schema 严格校验：所有工具输入 `#[serde(deny_unknown_fields)]`：见 [std_tools.rs](../../crates/deepagents/src/tools/std_tools.rs)
- state 可观测接口：CLI `tool --state-file ...` 已提供：见 [main.rs](../../crates/deepagents-cli/src/main.rs#L30-L180)

Phase 1 之后仍需持续对齐的 RESEARCH 契约点（不属于 Phase 1 的已实现范围）：large tool result offload、虚拟路径/路由模型（CompositeBackend）、summarization_event。
...
- **测试**：
  ...
  - Phase 1 E2E 计划文档已存在：[E2E_PHASE1.md](../e2e/E2E_PHASE1.md)
...
- ToolSchema/`ToolSpec`：工具的输入输出类型与元信息（名称/描述/输入 struct/输出 struct/JSON Schema）
...
- 对模型可见：基于 input struct 生成 JSON Schema 并注入 tool_specs（function calling）
- 回归方式（二选一，或同时）：
...
- JSON Schema 约束策略：
  - `required` 与可选字段匹配 serde（含 default）
  - `additionalProperties=false` 对齐 `deny_unknown_fields`
  - `enum`/`oneOf` 与内部枚举（如 grep.output_mode）一致
...
- 任务
  - 将 `ToolExecutionMiddleware` 从空 trait 扩展为可执行契约（例如：`before_tool`/`after_tool` 钩子或统一 `handle`）
...
- 任务
  ...
  - 将 [E2E_PHASE1.md](../e2e/E2E_PHASE1.md) 中的最小集合落地为可执行 smoke E2E
- 验收
  - `cargo test` 全量通过
...
- [E2E_PHASE1.md](../e2e/E2E_PHASE1.md)
...
- 技术设计基线：[TECH_DESIGN.md](../TECH_DESIGN.md)
- Phase 1 E2E 计划：[E2E_PHASE1.md](../e2e/E2E_PHASE1.md)
- Phase 0 参考：
  - Phase 0 详细计划：[ITERATION_PHASE0_DETAILED.md](ITERATION_PHASE0_DETAILED.md)
  - Phase 0 E2E 计划：[E2E_PHASE0.md](../e2e/E2E_PHASE0.md)
