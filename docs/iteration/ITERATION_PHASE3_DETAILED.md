...
适用范围：本计划面向 [ITERATION_PLAN.md](ITERATION_PLAN.md#L131-L157) 的 Phase 3。目标是提供可用的 ACP 服务端：会话、消息、工具调用、结果回传；并复用 Phase 1/2 的 tool schema、错误码语义与 execute 安全策略。
...
- Trait 优先：会话存储、审计、审批/策略、传输层都必须可插拔（第三方无需 fork）
...
- 黑盒 E2E 落地：以第三者视角驱动服务端进程并跑通测试计划（见 [E2E_PHASE3_ACP.md](../e2e/E2E_PHASE3_ACP.md)）
...
以当前代码为 ground truth，ACP server（HTTP + JSON v1）已实现最小闭环：见 [server.rs](../../crates/deepagents-acp/src/server.rs)。
...
本阶段剩余工作更多是“契约与回归强度/可插拔性”，而不是补齐从 0 到 1 的接口：

- 进一步冻结并文档化对外契约（字段、错误码、state_version 语义）与 E2E 覆盖（以 [E2E_PHASE3_ACP.md](../e2e/E2E_PHASE3_ACP.md) 为准）。
- 抽离传输层与业务层的 trait 边界（当前实现偏单体文件），为后续鉴权/限流/持久会话存储预留可插拔点。
...
- 黑盒 E2E：以第三者视角驱动 server（见 [E2E_PHASE3_ACP.md](../e2e/E2E_PHASE3_ACP.md)）
- 单元测试（建议但不替代 E2E）
  - SessionStore 并发与幂等
