...
适用范围：本计划面向 [ITERATION_PLAN.md](ITERATION_PLAN.md#L57-L98) 的 Phase 1.5。目标是在 Phase 1 已具备的“工具 + filesystem state 回填”基础上，补齐 Rust 版最关键的三项抽象（Runtime / Provider / SkillPlugin），并建立一个可回归的端到端闭环：用户消息 → provider 推理 → tool call → tool 执行 → 结果回填 → 最终响应。
...
- Phase 1.5 E2E 计划：[E2E_PHASE1_5.md](../e2e/E2E_PHASE1_5.md)
- 技术设计基线（含 Runtime/Provider/SkillPlugin 方向）：[TECH_DESIGN.md](../TECH_DESIGN.md)
- Phase 1 状态与 schema 基础：[ITERATION_PHASE1_DETAILED.md](ITERATION_PHASE1_DETAILED.md)

## 0. 当前系统基线（Phase 1.5 启动时）

以当前代码为 ground truth，Phase 1.5 的核心闭环已落地（本节用于校准“文档/计划”与实现一致）：

- Runtime：`SimpleRuntime` 循环（provider.step → tool_calls/skill_call → 执行 → 收敛），见 [simple.rs](../../crates/deepagents/src/runtime/simple.rs)
- Provider：`Provider` trait + `MockProvider`（脚本驱动回归），见 [protocol.rs](../../crates/deepagents/src/provider/protocol.rs) 与 [mock.rs](../../crates/deepagents/src/provider/mock.rs)
- Skills：`SkillPlugin` trait + 声明式 manifest 插件（manifest→tool_calls），见 [skills/protocol.rs](../../crates/deepagents/src/skills/protocol.rs) 与 [declarative.rs](../../crates/deepagents/src/skills/declarative.rs)
- CLI 端到端入口：`deepagents run ...` 与 `deepagents tool ... --state-file ...`，见 [main.rs](../../crates/deepagents-cli/src/main.rs#L30-L260)

Phase 1.5 之后仍需补齐/对齐的关键点（与 RESEARCH 的契约锚点直接相关）：

- Runner 事件流与 interrupt/resume：当前 `run` 以一次性 `RunOutput` 输出为主，未实现 Python CLI 的 `messages + updates + interrupt` 流式语义。
- PatchToolCalls：尚未实现运行前 tool_call 对齐修补（恢复历史/续跑的必备能力）。
- large tool result offload 与 summarization_event：尚未实现（需要在后续阶段按 RESEARCH 锚点落地）。
...
- 可回归测试：
  - E2E：至少覆盖 [E2E_PHASE1_5.md](../e2e/E2E_PHASE1_5.md) 中的“最小闭环 + provider 可替换 + tool call 解析鲁棒 + 错误/超时 + plugin 失败隔离（按选型）”
...
- 输入：messages、可用工具清单（名称/描述/JSON Schema）、当前 state 摘要、超时/重试参数
