...
适用范围：本计划面向 [ITERATION_PLAN.md](../iteration/ITERATION_PLAN.md) 的 Phase 1.5。目标是验证 Rust 版在 Phase 1.5 能形成“消息 →（provider 推理）→ tool call/skill call → tool 执行 → 结果回填（messages/state）→ 最终响应”的最小端到端闭环，并且 Runtime/Provider/SkillPlugin 三大抽象可替换、行为可回归。效果参考 Python 版本“闭环稳定、provider 可替换、tool 结果可回填并收敛为最终答复”的体验，但不依赖 Python 代码实现细节。
...
- Skills 插件：声明式 `DeclarativeSkillPlugin`（manifest → tool_calls）
  - manifest 与 merge_args 行为：[declarative.rs](../../crates/deepagents/src/skills/declarative.rs#L9-L80)
- 现有 Phase 1.5 CLI 级 E2E 测试（已覆盖的一部分能力）
  - 测试文件：[e2e_phase1_5_runtime.rs](../../crates/deepagents-cli/tests/e2e_phase1_5_runtime.rs)
  - 已覆盖：最小闭环、多轮 tool、无 tool 直接回答、provider 替换（mock2）、provider_timeout 分类、unknown tool 记录、schema 校验错误记录、路径越界/符号链接逃逸拒绝、is_directory 错误记录、execute allow-list 拒绝、max_steps_exceeded 分类、skill_not_found 分类、merge_args 覆盖、state write/edit/delete 可观测。
...
- `call(SkillCall)` 展开为 tool_calls，并将 `SkillCall.input` 作为 overlay 覆盖 manifest 的 `arguments`：[declarative.rs](../../crates/deepagents/src/skills/declarative.rs#L70-L79)
...
- E2E-PLUG-DECL-001：skill_call 展开为 tool_call 并执行
  - 现状：已覆盖：[phase1_5_declarative_skill_plugin_can_trigger_tool](../../crates/deepagents-cli/tests/e2e_phase1_5_runtime.rs#L209-L253)
- E2E-PLUG-DECL-002：skill_not_found（provider 发起不存在的 skill）
  - 现状：已覆盖：[phase1_5_skill_not_found_is_classified](../../crates/deepagents-cli/tests/e2e_phase1_5_runtime.rs#L574-L609)
...
- E2E-PLUG-DECL-003：merge_args overlay 覆盖 base
  - 现状：已覆盖：[phase1_5_declarative_skill_merge_args_overrides_base](../../crates/deepagents-cli/tests/e2e_phase1_5_runtime.rs#L610-L656)
