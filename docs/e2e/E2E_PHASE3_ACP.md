...
适用范围：本计划面向 [ITERATION_PLAN.md](../iteration/ITERATION_PLAN.md) 的 Phase 3：提供可用的 ACP 服务端（会话、消息、工具调用、结果回传），并复用 Phase 1/2 的 tool schema 与错误码。**本计划为黑盒 E2E**：只关注外部可观察行为与结果，不依赖实现细节、内部模块、源码结构或运行时选型。
...
**state_version 契约（必须固化）**

为确保第三方 client 可稳定实现“增量同步/幂等重试”，本计划要求对外固化 `state_version` 的最小语义：

- 同一 session 内 `state_version` 单调不减
- 若一次工具调用导致 `state`/`delta` 发生实际变化，则 `state_version` 必须递增
- 若一次调用失败且不产生任何状态变化，允许 `state_version` 保持不变（但必须稳定一致，E2E 用例需覆盖并锁定该行为）
