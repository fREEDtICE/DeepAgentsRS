...
- 范围（Phase 1 必测）
- 工具 schema 固化：工具入参/出参字段、字段命名、默认值行为、错误码集合、输出模式（grep）、截断信号、JSON Schema 可导出且与 serde 一致
  - FilesystemMiddleware state
    - files 快照写入：write/edit 后 state 中出现对应文件
    - reducer 合并：多次更新可合并、可覆盖、支持删除语义（对齐 Python：`null` 表示删除条目）
...
- read_file：不传 offset/limit 时默认值行为稳定（建议对齐 Python：offset=0、limit=100）
...
**E2E-SCHEMA-004：JSON Schema 与 serde 一致**

- 步骤：导出工具 JSON Schema（tool_specs 或离线快照），检查 `required`、`additionalProperties=false`、枚举值与默认值
- 断言：与工具输入 struct 的 serde 规则一致；未暴露未知字段
...
说明：本阶段建议直接把删除语义固化为 reducer 行为契约（对齐 Python：state patch 中某路径的值为 `null` 表示“删除该条目”，而不是把值设置为 JSON null）。

**E2E-STATE-DEL-001：null 删除 patch 能移除 state 条目**

- 步骤：写入 a.txt；通过 delete 工具或等价机制产生 deletion patch（如 `{"filesystem":{"files":{"<path>":null}}}`）
- 期望：state.files 不再包含 a.txt
- 断言：删除可重复（幂等）

**E2E-STATE-DEL-002：删除 patch 不应留下值为 null 的残留条目**

- 步骤：写入 a.txt；删除 a.txt；读取完整 state
- 断言：不存在 `a.txt: null` 这样的残留记录；应为“条目被移除”
...
- 期望：输出包含机器可断言的 truncation 指示字段（不依赖提示文本）
- 断言：用 offset/limit 能继续拉取后续内容；truncation 字段在“未截断/已截断”时取值稳定
...
- 步骤：read_file 传入 `~/secret.txt`
- 期望：拒绝
- 断言：错误码可分类（如 `invalid_path`）；不泄露宿主用户目录内容

**E2E-SEC-004：拒绝 Windows 盘符绝对路径**

- 步骤：read_file 传入 `C:\\Windows\\win.ini`
- 期望：拒绝
- 断言：错误码可分类（如 `invalid_path`）

**E2E-SEC-005：归一化后越界仍必须拒绝**

- 步骤：构造带重复分隔符/相对段的路径（例如 `<root>//a//..//../outside.txt`）
- 期望：拒绝
- 断言：不会被“自动修正”为 root 内路径
...
- 删除语义（必须明确）：对齐 Python reducer，`null` 表示删除条目，且删除后不应保留 `path: null` 残留
- read_file 默认值（必须明确）：建议对齐 Python（offset=0、limit=100），并用 E2E 固化
- truncation 信号形态（必须明确）：建议采用字段（机器可断言），并明确继续拉取方式（offset/limit 或 next_offset）
