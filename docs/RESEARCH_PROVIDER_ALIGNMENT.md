...
- CLI / ACP / runtime 只应持有统一抽象（如 `ToolSpec`、`ToolChoice`）。
- provider-native tools payload / `ToolsPayload` 只应存在于 provider 适配层，不应进入 ACP 协议或对外请求面。
- Rust 实现上应优先使用 typed enum / struct 表达 provider conversion surface，而不是把 provider-native 绑定收敛成匿名 JSON bag。
...
- ProviderStep 语义边界仍需持续关注：虽然当前实现已经通过 `AssistantMessageWithToolCalls` 补上“文本 + tool_calls 同响应”的主缺口，但这不意味着 `ProviderStep` 应继续吸收所有内容语义。`ProviderStep` 更适合作为“运行时控制流”边界；structured output、多模态 content blocks、reasoning content 这类“内容表示语义”应优先落在 `Message` / `RunOutput` / provider-specific normalized payload 中，只有在它们会改变 runtime 决策时，才应进入 step 层。
...
4. 持续守住 step 边界：当前已用 `AssistantMessageWithToolCalls` 覆盖“文本 + 工具调用同一响应”。后续 structured output / reasoning / multimodal 能力应优先进入内容模型而不是继续膨胀 `ProviderStep`；只有当某项能力确实改变 agent loop 控制流时，才评估扩展当前枚举或引入更稳定的 V2 surface。
