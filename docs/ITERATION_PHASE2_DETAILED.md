...
适用范围：本计划面向 [ITERATION_PLAN.md](file:///Users/dafengxu/Dev/deepagents/rust/docs/ITERATION_PLAN.md#L88-L109) 的 Phase 2。目标是在“产品层（CLI/运行入口）”把 `execute` 风险收敛到可控范围：deny-by-default + allow-list/审批，并提供可审计输出且不泄露敏感信息。

本计划基于当前系统现状（截至 Phase 1.5 完成）：

- `LocalSandbox` 已内置 allow-list + 危险 pattern 检测与管道/复合命令切分（属于默认 backend 实现细节）
- CLI 已支持 `--shell-allow` 参数，但缺少统一、可替换的“审批策略”抽象与配置体系
- 缺少对齐 Python 版本用例集合的系统化测试矩阵（尤其是危险模式、pipeline/compound、空输入、审计输出）
...
- 策略层缺失：没有 `ApprovalPolicy` trait 作为“产品层策略”统一入口（目前策略散落在 backend）
- 配置层缺失：缺少 CLI 配置项/环境变量约定（例如：默认策略、allow-list 来源、审批模式）
- 审计层缺失：缺少稳定的“可审计输出”（结构化、可重放、默认不泄露敏感信息）
- 测试层缺失：缺少对齐 Python 行为契约的测试矩阵（危险模式、pipeline/compound、空输入、允许/拒绝原因分类）
